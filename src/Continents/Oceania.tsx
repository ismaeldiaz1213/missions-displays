...
const Oceania: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMissionaries = async () => {
      try {
        const res = await fetch(`${API_ENDPOINT}missionaries/continent/oceania`);
  ...
  return (
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'linear-gradient(180deg, #F0F4F8 0%, #E8F1FC 100%)', overflow: 'hidden' }}>
      <ContinentHeader title="Oceanía" count={missionaries.length} />
        ...
        <Box sx={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          <Paper sx={{ flex: 1, display: 'flex', flexDirection: 'column', p: 1.5, background: 'linear-gradient(135deg, #FFFFFF 0%, #F0F4F8 100%)', borderRadius: '12px', border: '2px solid rgba(37,99,235,0.1)', boxShadow: '0 15px 30px -5px rgba(30,58,138,0.15)' }}>
            ...
            <Box sx={{ flex: 1, minHeight: 0 }}>
              <LeafletRegionalMap centerLat={OCEANIA_LAT_CENTER} centerLong={OCEANIA_LON_CENTER} missionaries={currentMissionaries} selectedMissionaryId={selectedMissionaryId} onMissionarySelect={setSelectedMissionaryId} />
            </Box>
          </Paper>
        </Box>
      </Box>
